...
/* Terminal strucure */
typedef struct mul_ter_info_t{
    uint8_t keyboard_buffer[KEYBOARD_MAX_SIZE];
    int32_t screen_x;
    int32_t screen_y;
    int32_t active_pid;
    int32_t buffer_pos;
    volatile int32_t enter_pressed;
    int32_t num_process_this_terminal;
    volatile int32_t count;
    volatile int32_t req_count;
} mul_ter_info_t;

mul_ter_info_t terminal_arr[MAX_NUM_TERMINALS];

//void rtc_open_write_test();

/* Getters and setters for cur_terminal */
int32_t get_cur_terminal();
void set_cur_terminal(int32_t terminal);

/* Getters and setters for cur_terminal_running */
