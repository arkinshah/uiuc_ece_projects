...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
		...
		while(mbi->mods_count == 0){printf("Number of modules loaded are %d\n", mbi->mods_count);}
        while (mod_count < mbi->mods_count) {
            printf("Module %d loaded at address: 0x%#x\n", mod_count, (unsigned int)mod->mod_start);
            bootblk_start = mod->mod_start; // fetch bootblock address pointer
    ...
    /* Initialize File Systems Memory */
    file_system_init();
    ...
    for(i = 0; i < MAX_NUM_TERMINALS; i++){
        //terminal_arr[i].screen_x = 0;
        //terminal_arr[i].screen_y = 0;
        terminal_arr[i].active_pid = -1;
        terminal_arr[i].num_process_this_terminal = 0;
    }
    //set_cursor(0,0);
