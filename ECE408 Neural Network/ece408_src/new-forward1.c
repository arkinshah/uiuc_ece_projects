...
__global__ void matrixMultiply(float *A, float *B, float *C, int numAColumns, int numCRows, int numCColumns)
{
  ...
  for (int i = 0; i < (TILE_WIDTH + numAColumns - 1)/(TILE_WIDTH); i++) {
    ...
    if((i*TILE_WIDTH+tx) < numAColumns){
      Ads[ty][tx] = A[Row*numAColumns+(i*TILE_WIDTH+tx)];
    }
    else{
      Ads[ty][tx] = 0;
    }
...
void gemm(int w_base, int M, int H_unroll, float *Y, float* X_unroll, float* _K) {
    dim3 gridDim (ceil((float) H_unroll/TILE_WIDTH), ceil((float) M/TILE_WIDTH));
    dim3 blockDim (TILE_WIDTH, TILE_WIDTH);
    matrixMultiply<<<gridDim, blockDim>>>(_K, X_unroll, Y, w_base, M, H_unroll);
}
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    for(int i = 0; i <= B; i++){
      ...
      gemm(w_base, M,  H_unroll, Y+i*(M*H_unroll), X_unroll, _K);
    }
    cudaFree(X_unroll);
